WITH flight_sequence AS (
    SELECT 
        ...
        LAG(departure_airport) OVER (PARTITION BY aircraft_id ORDER BY actual_departure) AS previous_origin,
        LAG(departure_temp_c) OVER (PARTITION BY aircraft_id ORDER BY actual_departure) AS previous_origin_temp_c,
        LAG(arrival_delay_minutes) OVER (PARTITION BY aircraft_id ORDER BY actual_departure) AS previous_arrival_delay
